class ClassroomA < ActiveRecord::Base
  ...
  def self.conflict?
    ...
    modules.map do |conflict|
      if conflict.include?("Classroom A")
        conflicts << "Yes"
      else
        conflicts << "No"
      end
  ...
  def self.cohort(cohort_num)
    find_teacher
    ClassroomA.first.update(cohort: cohort_num.ljust(14, " "), teacher: @teacher[0])
  end

  def self.tbd
    ClassroomA.first.update(cohort: "Check Today!", teacher: "I Dunno :P")
  end

  def self.conflicting_cohorts
    ClassroomA.first.update(cohort: "Conflict  ", teacher: "Help    ")
  end
